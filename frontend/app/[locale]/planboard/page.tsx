...
export default function PlanboardPage() {
    ...
    const handleDragEnd = (event: DragEndEvent) => {
        ...
        // active.id = "event-123" -> берем 123
        const eventId = Number(active.id.toString().replace('event-', ''));
        // over.id = "urgent-pet-1" -> парсим тип
        const [targetType, , petIdStr] = over.id.toString().split('-'); // "urgent-pet-1" -> ["urgent", "pet", "1"]
        
        // Находим сам ивент в стейте
