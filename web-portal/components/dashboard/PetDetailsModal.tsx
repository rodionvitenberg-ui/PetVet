...
import { 
  X, Activity, FileText, ChevronLeft, ChevronRight, ImageIcon, Calendar, Trash2
} from 'lucide-react';
...
export default function PetDetailsModal({ petId, isOpen, onClose }: { petId: number | null, isOpen: boolean, onClose: () => void }) {
  ...
  const handleDelete = async () => {
      ...
      const confirmed = window.confirm(`Вы действительно хотите удалить профиль питомца "${pet.name}"? Это действие нельзя отменить.`);
      if (!confirmed) return;
                    ...
                    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
                        ...
                        <div className="pt-8 mt-8 border-t border-gray-100">
                            ...
                            <p className="text-center text-[10px] text-gray-400 mt-2">
                                Это действие нельзя отменить. Все данные будут стерты.
                            </p>
                        </div>
                    </div>
                )}
                                                ...
                                                <div className="flex justify-between items-start">
                                                    <div>
                                                        ...
                                                        <p className="text-xs text-gray-500 mt-0.5 font-medium">
                                                            ...
                                                            {event.created_by_clinic && ` • ${event.created_by_clinic}`}
                                                        </p>
                                                    </div>
